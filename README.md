...
A reusable scaffold for setting up automated API testing on any REST API, built on **Postman + Newman** and driven by four custom Claude Code agents.

Clone it as the starting point for a new QA project, run the setup command, and you have a working test suite. The agents adapt to whatever OpenAPI spec, base URL, and credentials you give them — **nothing is hardcoded to a specific project**.

---

## Table of contents

<!-- Clickable links without underline in IDE preview. GitHub web reverts to underlined (it strips inline styles). -->

- <a href="#what-this-is" style="text-decoration: none;">What this is</a>
- <a href="#prerequisites" style="text-decoration: none;">Prerequisites</a>
- <a href="#the-agents" style="text-decoration: none;">The agents</a>
- <a href="#full-workflow" style="text-decoration: none;">Full workflow</a>
  - <a href="#data-flow" style="text-decoration: none;">Data flow</a>
  - <a href="#when-to-run-what" style="text-decoration: none;">When to run what</a>
  - <a href="#day-1-first-time-setup" style="text-decoration: none;">Day 1: First-time setup</a>
  - <a href="#day-2-onwards-spec-stays-in-sync-automatically" style="text-decoration: none;">Day 2 onwards: Spec stays in sync automatically</a>
  - <a href="#when-a-new-jira-ticket-lands" style="text-decoration: none;">When a new Jira ticket lands</a>
- <a href="#command-details" style="text-decoration: none;">Command details</a>
  - <a href="#qa-api-test-setup" style="text-decoration: none;"><code>/qa-api-test-setup</code></a>
  - <a href="#main-collection-folder-structure" style="text-decoration: none;">Main-collection folder structure</a>
  - <a href="#qa-test-ticket-jira-key" style="text-decoration: none;"><code>/qa-test-ticket &lt;JIRA-KEY&gt;</code></a>
...
- <a href="#what-gets-generated-by-run-testssh" style="text-decoration: none;">What gets generated by <code>./run-tests.sh</code></a>
- <a href="#env-keys" style="text-decoration: none;"><code>.env</code> keys</a>
- <a href="#running-tests" style="text-decoration: none;">Running tests</a>
- <a href="#when-tests-fail-auto-fix-first-then-debug-then-log-to-jira" style="text-decoration: none;">When tests fail (auto-fix → debug → Jira)</a>
- <a href="#common-scenarios" style="text-decoration: none;">Common scenarios</a>
...
All four agents are slash commands that live in `.claude/commands/` and are invoked from inside Claude Code.
...
> [!TIP]
> If any plugin is missing when you run `/qa-api-test-setup`, Phase 0 stops and tells you exactly which one to enable and how. You won't get silent failures partway through.
...
```
   ...
   │  project-snapshot-YYYY-MM-DD.json        (drift baseline)        │
...
11. Write the initial `project-snapshot-YYYY-MM-DD.json` baseline.
...
The **"first-day setup"** agent. When you start QA work on a brand new API project, this is the only thing you need to run to bootstrap everything from zero.

It handles every annoying piece of test-suite setup that nobody wants to do by hand:

- Reads the OpenAPI spec (Scalar, Swagger UI, Redoc, raw JSON/YAML — auto-detected).
- Asks a handful of short questions (only what it cannot figure out on its own).
- Builds a full Postman collection with login, happy-path tests, and the full negative matrix per endpoint.
- Writes a `.env` file, a Newman run script, and a baseline snapshot file (used later by sync to detect drift).
- Creates a shared Postman environment so manual GUI runs work without extra config.

After it finishes, you have a runnable test suite that produces HTML reports — no manual Postman clicking required.
...
Built-in guard: before doing anything, it scans the project directory for setup-owned artifacts (`<project-slug>-collection.json`, `project-snapshot-*.json`, main collection in Postman cloud). Shared infrastructure and ticket subfolders are NOT counted — those are owned by other workflows.
...
- `project-snapshot-<today>.json` (baseline for future drift detection)
...
│   ├── 03. List Orders
│   ├── 04. Create Order
│   └── ...                          # named <NN>. <op.summary> in audited flow order; status-only assertions
...
> [!NOTE]
> Every module's `Negative` folder ALWAYS has all 4 sub-folders (`Auth Failures`, `Validation Failures`, `Resource Errors`, `Security Probes`), even if some are empty for that module. Keeps the layout visually consistent so users always know where to look.
>
> The sub-folder name describes the **category of failure** the tests inside cover, applied to the **parent module's** endpoints — so `Orders > Negative > Auth Failures` holds the auth-failure tests *for Orders endpoints* (not tests of the Auth module).
...
Key properties:
- **`Happy Path - All Endpoints`** is a fast smoke pass. One valid-data request per `(method, path)`, named `<NN>. <op.summary>` (sequential index + the OpenAPI op's `summary`, e.g. `02. Login`, `05. Create Order`). Status-only assertion (`2xx`). Requests are ordered by the **dependency audit** (Login runs before any token-protected endpoint, Create Order runs before any endpoint needing `{{testOrderId}}`, etc.), not by spec order. This folder is in addition to — not replacing — the module folders.
...
The **per-ticket testing agent**. Every time the Jira board lights up with a new feature, bug, or story that touches the API, you run this once with the ticket key — it builds you a dedicated test collection just for that ticket.

What it does, in order:

1. **Reads the ticket** directly from Jira (or accepts pasted content if the Atlassian plugin isn't connected).
2. **Figures out which endpoints are in scope** by matching ticket terminology against the OpenAPI spec.
3. **Plans the Setup / Happy Path / Positive / Negative scenarios** with audited flow order.
4. **Hard-stops** to ask for any credentials it needs but doesn't have — no silent failures.
5. **Builds a standalone collection** in Postman that maps 1:1 to the ticket's acceptance criteria.

Each ticket gets its own collection named like `PROJ-123: <Feature Name>`, so your main test suite stays clean and your per-ticket work stays organized.
...
│   ├── 01. Create Order
│   ├── 02. Get Order By ID
│   └── ...                           # named <NN>. <op.summary>, status-only assertions
├── Positive
...
- **Happy Path - All Endpoints** is scoped to only the endpoints this ticket touches — one valid-data request per `(method, path)`, named `<NN>. <op.summary>` (sequential within the ticket's Happy Path folder), status-only assertions. Variable chaining (`pm.environment.set`) stays in Setup; Happy Path doesn't set vars.
...
The **spec watcher**. Your API is a moving target — developers ship new endpoints, change required fields, deprecate old routes. Without this agent, your tests would silently rot until something breaks in production.

This agent compares the current OpenAPI spec against the last snapshot it took, figures out exactly what changed, and updates your Postman collections to match:

- **New endpoints** → auto-generated happy + full negative matrix.
- **Changed endpoints** → request bodies and params refreshed; hand-written test assertions preserved untouched.
- **Removed endpoints** → renamed to `[DEPRECATED]` (not deleted) so you keep the history.
- **Ticket collections that reference changed endpoints** → also refreshed, so per-ticket work stays aligned with the latest API even when you haven't touched a ticket in weeks.
...
├── project-snapshot-YYYY-MM-DD.json           # GENERATED. Drift baseline, updated by /qa-api-sync
├── <project-slug>-collection.json         # GENERATED. Main Postman collection export
├── Jira Tickets/                           # GENERATED. Ticket-owned subfolder tree
│   └── <KEY>/                              # One folder per Jira ticket built
│       ├── collection.json                 # Ticket Postman collection export
│       └── snapshot-YYYY-MM-DD.json       # Per-ticket scoped snapshot
...
**Naming conventions:**
- **`<project-slug>`** — a lowercase, hyphen-free version of your project name (chosen during `/qa-api-test-setup`, e.g. `myproject`).
- **`<jira-key>`** — the lowercased ticket key (e.g. `PROJ-123` becomes `proj123`).
- **`<openapi-spec>`** — whatever you named your spec file. The agents do not care about the filename, only the path/URL stored in `.env`.
...
- **409 conflict on a unique field** -> patches the body with a Mode A random generator (`{{$randomEmail}}`, `{{$randomUUID}}`, etc.), re-runs.
...
- Bail the Newman run on the first failure. The full suite always runs to completion so the HTML report captures every failure in one pass. `--bail` is reserved for manual single-bug triage by a human (see `.claude/commands/qa-debugging-playbook.md` § 5).
...
For failures that survived auto-fix, the agent surfaces the debugging playbook (`.claude/commands/qa-debugging-playbook.md` — canonical reference shared by all four agents):
...
4. **Match against common HTTP patterns** - 401 cascade after login fails, 422 from wrong state, 409 from a hardcoded unique-field value (use Mode A random gen), 400 from spec drift (run `/qa-api-sync`), etc. Each has a known fix-path.
...
The preflight guard will stop and refuse to proceed. It lists what it detected and tells you which command you actually wanted:

- `/qa-api-sync` for spec updates
- `/qa-test-ticket` for ticket work
- `./run-tests.sh` to just run the suite

To override (only if you genuinely want a fresh duplicate collection), type the exact phrase **`force re-setup`** when prompted. Generic `y/n` is intentionally not accepted.
