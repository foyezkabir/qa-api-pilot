...
Test script style follows `/qa-api-test-setup` Phase 5c rules (let-only, status-first assertion, `{{$timestamp}}` for unique fields, `pm.collectionVariables.set` for chaining in the main collection). Matrix-specific scripts come from the matrix doc.
